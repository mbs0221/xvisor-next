...
static void *imsic_cpu_page_virt(unsigned int cpu,
				 unsigned int guest_index)
{
	struct imsic_handler *handler = &per_cpu(imsic_handlers, cpu);
	struct imsic_global_config *global;
	...
	return local->msi_va + (guest_index * IMSIC_MMIO_PAGE_SZ);
}
...
{
	csr_write(CSR_SSETEIENUM, (unsigned long)data);
}
...
{
	csr_write(CSR_SCLREIENUM, (unsigned long)data);
}
...
static void imsic_ids_local_sync(struct imsic_priv *priv)
{
	...
	for (i = 1; i <= priv->global.nr_ids; i++) {
		...
		if (test_bit(i, priv->ids_enabled_bimap))
			csr_write(CSR_SSETEIENUM, i);
		else
			csr_write(CSR_SCLREIENUM, i);
	}
	vmm_spin_unlock_irqrestore_lite(&priv->ids_lock, flags);
}
				...
				const struct vmm_cpumask *mask)
{
	...
	for_each_cpu(cpu, mask) {
		...
		if (!msi_va) {
			vmm_lwarning("imsic",
				     "CPU%d: failed to get MSI address\n", cpu);
			continue;
		}
		vmm_writel(hwirq, msi_va);
	}
}
...
static int __init imsic_ipi_domain_init(struct imsic_priv *priv)
{
	...
	if (priv->global.nr_ids < priv->base_ipi) {
		return VMM_EINVALID;
	}
	if (priv->global.nr_ids < (priv->base_ipi + priv->nr_ipis - 1)) {
		return VMM_EINVALID;
	}
	...
	for (i = 0; i < priv->nr_ipis; i++) {
		rc = vmm_host_irqdomain_create_mapping(priv->ipi_domain, i);
		if (rc < 0) {
			vmm_lerror("imsic",
				   "failed to create IPI%d mapping\n", i);
...
static int imsic_irq_domain_alloc(struct vmm_host_irqdomain *d,
...
static void imsic_irq_domain_free(struct vmm_host_irqdomain *d,
				  unsigned int hwirq, unsigned int nr_irqs)
{
	struct imsic_priv *priv = d->host_data;
...
static int __init imsic_init(struct vmm_devtree_node *node)
{
	...
	if (vmm_devtree_read_u32_atindex(node, "riscv,ipi-range",
					 &priv->base_ipi, 0))
		priv->base_ipi = 0;
	if (vmm_devtree_read_u32_atindex(node, "riscv,ipi-range",
					 &priv->nr_ipis, 1))
...
#ifdef CONFIG_SMP
	if (priv->nr_ipis)
		vmm_init_printf("%s: providing %d IPIs starting at interrupt %d\n",
				node->name, priv->nr_ipis, priv->base_ipi);
#endif
